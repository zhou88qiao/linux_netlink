	...
	for {
		...
		for _, m := range msgs {
			...
			if m.Header.Type == syscall.NLMSG_ERROR {
				fmt.Printf("err11111")
				return nil, fmt.Errorf("err1111")
				//ParseNetlinkError(m.Data)
			}
...
func ParseInetDiagMsg(b []byte) (*InetDiagMsg, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to unmarshal inet_diag_msg")
...
func main() {
	...
	idr := NewInetDiagReqV2(syscall.AF_INET)
	ret, err := NetlinkInetDiag(idr)
	if err != nil {
		fmt.Println("error:" + err.Error())
	} else {
		fmt.Println(len(ret))
		for _, msg := range ret {
			state := (TCPState)(msg.State).String()
			sip := msg.SrcIP().String()
			sport := msg.SrcPort()
			dip := msg.DstIP().String()
			dport := msg.DstPort()
			uid := msg.UID
			inode := msg.Inode
			fmt.Printf("uid:%v,state:%v,sip:%v,sport:%v,dip:%v,dport:%v,inode:%v\n", uid, state, sip, sport, dip, dport, inode)
		}
	}
