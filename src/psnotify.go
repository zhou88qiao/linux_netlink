...
func getProcessStatus(pid uint32) (error, string) {
	...
	return nil, strings.Join(strParts, " ")
}

func getProcessExe(pid uint32) (error, string) {
...
func getProcessCwd(pid uint32) (error, string) {
...
func newProc(pid uint32) (err error, proc Proc) {
	...
	err, exe := getProcessExe(pid)
	if err == nil {
		proc.exe = exe
	}

	err, cwd := getProcessCwd(pid)
	if err == nil {
		proc.cwd = cwd
	}
	...
	for {
		...
		case C.PROC_EVENT_FORK:
			...
			if (filter == "" && filterarg == "") || proc.Matches(filter, filterarg) {
				fmt.Printf("Fork: %+v -> process exe:%s, cmdline:%s, cwd:%s, uid:%v\n", ev.data.(ForkEvent), proc.exe, proc.cmdline, proc.cwd, proc.uid.real)
			}
		case C.PROC_EVENT_EXEC:
			_, proc := newProc(ev.data.(ExecEvent).pid)
			if (filter == "" && filterarg == "") || proc.Matches(filter, filterarg) {
				fmt.Printf("Exec: %+v -> process exe:%s, cmdline:%s, cwd:%s, uid:%v\n", ev.data.(ExecEvent), proc.exe, proc.cmdline, proc.cwd, proc.uid.real)
			}
		case C.PROC_EVENT_EXIT:
			_, proc := newProc(ev.data.(ExitEvent).pid)
			if (filter == "" && filterarg == "") || proc.Matches(filter, filterarg) {
				fmt.Printf("Exit: %+v -> process exe:%s, cmdline:%s, cwd:%s, uid:%v\n", ev.data.(ExitEvent), proc.exe, proc.cmdline, proc.cwd, proc.uid.real)
			}
		case C.PROC_EVENT_UID:
			_, proc := newProc(ev.data.(UidChangeEvent).pid)
			if (filter == "" && filterarg == "") || proc.Matches(filter, filterarg) {
				fmt.Printf("Uid: %+v -> process exe:%s, cmdline:%s, cwd:%s, uid:%v\n", ev.data.(UidChangeEvent), proc.exe, proc.cmdline, proc.cwd, proc.uid.real)
			}
		}
	}
}
